import { Button } from "@/components/ui/button";
import { IconBrandCashapp, IconBrandPaypal } from "@tabler/icons-react";
import {
  Dialog,
  DialogClose,
  DialogContent,
  DialogDescription,
  DialogFooter,
  DialogHeader,
  DialogTitle,
  DialogTrigger,
} from "@/components/ui/dialog";

export default function TradeDepositAccounts() {
  return (
    <div className="">
      <div className="flex items-center justify-between lg:px-10">
        <Button size={"lg"} variant={"outline"}>
          <IconBrandPaypal />
          <a href="https://paypal.me/yourbusinessname" target="_blank">
            PayPal
          </a>
        </Button>
        <Button size={"lg"} variant={"outline"}>
          <IconBrandCashapp />
          <a href="https://cash.app/$YourBrandName;" target="_blank">
            Cash App
          </a>
        </Button>
        <Dialog>
          <DialogTrigger asChild>
            <Button size={"lg"} variant={"outline"}>
              <svg
                xmlns="http://www.w3.org/2000/svg"
                width="24"
                height="24"
                viewBox="0 0 24 24"
              >
                <path
                  fill="currentColor"
                  d="M13.559 24h-2.841a.483.483 0 0 1-.483-.483v-2.765H5.638a.667.667 0 0 1-.666-.666v-2.234a.67.67 0 0 1 .142-.412l8.139-10.382h-7.25a.667.667 0 0 1-.667-.667V3.914c0-.367.299-.666.666-.666h4.23V.483c0-.266.217-.483.483-.483h2.841c.266 0 .483.217.483.483v2.765h4.323c.367 0 .666.299.666.666v2.137a.67.67 0 0 1-.141.41l-8.19 10.481h7.665c.367 0 .666.299.666.666v2.477a.667.667 0 0 1-.666.667h-4.32v2.765a.483.483 0 0 1-.483.483"
                />
              </svg>
              Zelle
            </Button>
          </DialogTrigger>
          <DialogContent>
            <DialogHeader>
              <DialogTitle>
                Zelle Email Address or U.S. Mobile Number:
              </DialogTitle>
              <DialogDescription>
                Send money with Zelle with any of the two provided information.
              </DialogDescription>
              <div className="mt-5 space-y-2">
                <p>
                  <strong>Email:</strong> yourbusiness@email.com
                </p>
                <p>
                  <strong>Phone Number:</strong> 123-456-7890
                </p>
              </div>
            </DialogHeader>
          </DialogContent>
        </Dialog>
      </div>
    </div>
  );
}
