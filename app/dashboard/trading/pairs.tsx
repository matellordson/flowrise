...
export default async function Pairs({ accountID }: { accountID: string }) {
  ...
  const xrpPrice = await fetchUSDTPrice();
  ...
  const getSignal =
    (await sql`SELECT id, pair, amount FROM signal`) as dataType[];
                ...
                <AlertDialogContent>
                  ...
                  <AlertDialogFooter>
                    ...
                    <AlertDialogAction
                      onClick={async () => {
                        "use server";
                        await sql`UPDATE trading_accounts SET balance = balance + ${data.amount} WHERE id = ${accountID}`;
                      }}
                    >
                      Trade
                    </AlertDialogAction>
